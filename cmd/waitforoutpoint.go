...
var waitforoutpointCmd = &cobra.Command{
	Use:   "waitforoutpoint",
...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		hash, _ := cmd.PersistentFlags().GetString("hash")
		index, _ := cmd.PersistentFlags().GetUint32("index")
		...
		for {
			...
			var found bool
			for _, input := range txWithBlock.GetTx().GetInputs() {
				outpoint := input.GetPreviousOutput()
				if outpoint.GetHash() == hash && outpoint.GetIndex() == index {
					found = true
					break
				}
			}
...
func init() {
	rootCmd.AddCommand(waitforoutpointCmd)

	waitforoutpointCmd.PersistentFlags().String("hash", "", "")
	waitforoutpointCmd.PersistentFlags().Uint32("index", 0, "")
}
